...
describe('getEpisodes', () => {
    test('response schema', async () => {
        const res = await (0, _1.getEpisodes)();
    ...
    test('get by filter', async () => {
        const res = await (0, _1.getEpisodes)({ episode: 'S01E01' });
        res.data.results?.forEach((item) => {
            expect(item.episode).toContain('S01E01');
        });
    });
    test('pagination', async () => {
        const res = await (0, _1.getEpisodes)({ page: 2 });
...
describe('getEpisode', () => {
    test('get by ID', async () => {
        const res = await (0, _1.getEpisode)(1);
        expect(res.data.id).toBe(1);
    });
    test('get by IDs', async () => {
        const res = await (0, _1.getEpisode)([1, 2]);
